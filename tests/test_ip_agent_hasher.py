...
def test_append_provenance_entries_persists_semantic_fingerprint_and_binary_digest(tmp_path: Path) -> None:
    artifact = tmp_path / "track.wav"
    artifact.write_bytes(b"v1")
    log_path = tmp_path / "provenance.jsonl"
    semantic = {"audio": {"tempo_estimate": 124}, "visual": {"dominant_palette_hash": "abc123"}}
    ...
    entries = append_provenance_entries(
        ...
        agent="ip_agent",
        log_path=log_path,
        semantic_fingerprints={str(artifact): semantic},
    )

    assert len(entries) == 1
    assert entries[0]["semantic_fingerprint"] == semantic
    assert entries[0]["binary_digest_sha256"] == entries[0]["sha256"]
